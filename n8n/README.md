...
The workflow foundation now uses multiple live RSS sources for the active focus while keeping extraction, synthesis, and database writes mocked. Feed items are filtered through the configured lookback window, then normalized into plain-text, prompt-safe source documents with bounded content length before they move further through the workflow.
...
The next workflow slice should expand beyond RSS-only source coverage and replace the mocked extraction and synthesis steps with real integrations.
